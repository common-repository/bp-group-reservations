...
function bp_group_reservation_create_reservation( $user_email, $group_id, $extras = false ) {
	...
	// Can be overridden with the `group_reservation_can_manage_reservations` filter
	if( ! is_super_admin() && ! apply_filters( 'group_reservation_can_manage_reservations', groups_is_user_admin( bp_loggedin_user_id(), $group_id ), $group_id ) ) {
		return false;
	}
...
/**
 * Delete all reservations for a particular user. Restricted to super admins only.
 */
